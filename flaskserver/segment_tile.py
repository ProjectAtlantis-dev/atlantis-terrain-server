...
def _load_cached_tile(db, tile_id):
    row = db.execute(
        "SELECT t.x_min, t.x_max, t.heightmap, x.texture "
    ...
    heightmap = np.frombuffer(
        zlib.decompress(row[2]), dtype=np.float32
    ).reshape((GRID_N, GRID_N)).copy()
    rgb = np.asarray(Image.open(io.BytesIO(row[3])).convert("RGB"))
    return rgb, heightmap, float(row[1] - row[0])
...
def main():
    ...
    with sqlite3.connect(args.db) as db:
        rgb, heightmap, tile_size_m = _load_cached_tile(db, args.tile_id)
