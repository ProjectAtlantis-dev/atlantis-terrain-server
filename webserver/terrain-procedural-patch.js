...
import { Heightfield } from './procedural-runtime/world/Heightfield.ts';
import { runScatter } from './procedural-runtime/gpu/passes/Scatter.ts';
import { GroundRing } from './procedural-runtime/vegetation/GroundRing.ts';
import { Forests } from './procedural-runtime/vegetation/Forests.ts';
import { buildVegLibrary } from './procedural-runtime/vegetation/VegLibrary.ts';
import { WorldSeed } from './procedural-runtime/core/Seed.ts';
import { updateSunUniforms } from './procedural-runtime/render/VegMaterials.ts';
import { mountSeasonUI } from './procedural-runtime/render/Season.ts';
import { setWindContext } from './procedural-runtime/render/Wind.ts';
import { parseTileId } from './procgen/chunk-window.js';
import {
  DEFAULT_PROCGEN_SOURCE_DEPTH,
...
// Keep detail active throughout the configured Osprey envelope (141 m/s).
// Separate enter/exit thresholds retain a safety gate for future faster
// vehicles without flickering when speed hovers near the boundary.
const DETAIL_SPEED_DISABLE_MPS = 150;
const DETAIL_SPEED_ENABLE_MPS = 130;
...
function collectTiles(terrainRoot, sourceDepth) {
  const tiles = [];
  for (const mesh of terrainRoot.children) {
    const input = mesh.userData?.scatterInput;
    const address = parseTileId(input?.tileId);
    if (!address || address.depth !== sourceDepth || !input?.hm
        || !Array.isArray(input.bbox) || input.res < 2) continue;
...
// Preserve the original depth-page vertex lattice for GPU grounding. Scatter
// candidates are generated on the GPU, so raycasting every rock is not viable;
// this compact mosaic lets the shader perform the exact same triangle lookup as
// terrain-mesh-builder without passing through the smoothed 3 m field grid.
function buildExactGroundGrid(descriptor, sourceTiles) {
  const tiles = sourceTiles.filter(tile => (
    tile.xMax >= descriptor.xMin && tile.xMin <= descriptor.xMax
    && tile.yMax >= descriptor.yMin && tile.yMin <= descriptor.yMax
  ));
  if (tiles.length === 0) return null;
  const sourceRes = tiles[0].res;
  const stepX = (tiles[0].xMax - tiles[0].xMin) / (sourceRes - 1);
  const stepY = (tiles[0].yMax - tiles[0].yMin) / (sourceRes - 1);
  const compatible = tiles.every(tile => (
    tile.res === sourceRes
    && Math.abs(((tile.xMax - tile.xMin) / (tile.res - 1)) - stepX) < 1e-5
    && Math.abs(((tile.yMax - tile.yMin) / (tile.res - 1)) - stepY) < 1e-5
  ));
  if (!compatible) return null;

  const minX = Math.min(...tiles.map(tile => tile.xMin));
  const minY = Math.min(...tiles.map(tile => tile.yMin));
  const maxX = Math.max(...tiles.map(tile => tile.xMax));
  const maxY = Math.max(...tiles.map(tile => tile.yMax));
  const columns = Math.round((maxX - minX) / stepX) + 1;
  const rows = Math.round((maxY - minY) / stepY) + 1;
  // PATCH_RES² is the preallocated persistent GPU capacity. A 768 m window
  // spans at most 3×3 of today's 659 m / 65-vertex depth-12 pages (193²).
  if (columns * rows > PATCH_RES * PATCH_RES) return null;

  const heights = new Float32Array(columns * rows);
  heights.fill(Number.NaN);
  for (const tile of tiles) {
    const columnBase = Math.round((tile.xMin - minX) / stepX);
    const rowBase = Math.round((tile.yMin - minY) / stepY);
    for (let row = 0; row < tile.res; row++) {
      const dst = (rowBase + row) * columns + columnBase;
      const src = row * tile.res;
      heights.set(tile.hm.subarray(src, src + tile.res), dst);
    }
  }
  // Do not publish a partially resident ground surface. Keeping the previous
  // mosaic here would ground new rocks against stale geography.
  for (let index = 0; index < heights.length; index++) {
    if (!Number.isFinite(heights[index])) return null;
  }
  return {
    heights,
    columns,
    rows,
    // Coordinates are local to the procedural root. Ground rows retain the
    // terrain mesh's +northing order; Heightfield maps procedural +z back to -y.
    originX: minX - descriptor.centerX,
    originY: minY - descriptor.centerY,
    stepX,
    stepY,
    sourceTiles: tiles.map(tile => tile.id).sort(),
  };
}
...
// the same EPSG:3413 position before the procedural frame's north flip is applied.
...
function buildWindow(descriptor, sourceTiles, fieldSources) {
  ...
  const exactGround = buildExactGroundGrid(descriptor, sourceTiles);
  if (!exactGround) return null;
  return {
    descriptor,
    heights,
    classifier,
    ...
    stats: {
      ...
      grounding: {
        mode: 'depth-page-triangles',
        upload: 'ordered-compute-copy',
        columns: exactGround.columns,
        rows: exactGround.rows,
        metresPerSampleX: Number(exactGround.stepX.toFixed(3)),
        metresPerSampleY: Number(exactGround.stepY.toFixed(3)),
        sourceTiles: exactGround.sourceTiles,
      },
    },
  };
}
...
export class TerrainProceduralPatch {
  constructor(terrainRoot, seedN, {
    ...
    this.procgenSourceDepth = DEFAULT_PROCGEN_SOURCE_DEPTH;
  ...
  setWorldIdentity({ worldSeed, procgenVersion, procgenSourceDepth } = {}) {
    const nextSeed = Number(worldSeed);
    const nextVersion = Number(procgenVersion);
    const nextSourceDepth = procgenSourceDepth == null
      ? this.procgenSourceDepth
      : Number(procgenSourceDepth);
    if (!Number.isInteger(nextSeed) || nextSeed < 0 || nextSeed > 0xffffffff
        ...
        || !Number.isInteger(nextSourceDepth) || nextSourceDepth < 0 || nextSourceDepth > 20) {
      return false;
    }
    if (this.ready || this.building) {
      const matches = this.seedN === (nextSeed >>> 0)
        && this.procgenVersion === (nextVersion >>> 0)
        && this.procgenSourceDepth === nextSourceDepth;
      if (!matches) {
        this.log('error', 'patch.world-identity.late-change-rejected', {
          active: {
            worldSeed: this.seedN,
            procgenVersion: this.procgenVersion,
            procgenSourceDepth: this.procgenSourceDepth,
          },
          requested: { worldSeed: nextSeed, procgenVersion: nextVersion, procgenSourceDepth: nextSourceDepth },
        });
      }
      return matches;
    }
    ...
    this.procgenVersion = nextVersion >>> 0;
    this.procgenSourceDepth = nextSourceDepth;
    this.log('info', 'patch.world-identity', {
      worldSeed: this.seedN,
      procgenVersion: this.procgenVersion,
      procgenSourceDepth: this.procgenSourceDepth,
    });
  ...
  async _ensureVegLibrary(renderer, seed) {
    if (!this.vegLibrary) {
      this.vegLibrary = await buildVegLibrary(
        renderer,
        seed,
        (progress, message) => this.log('info', 'patch.library', { progress, message }),
        { treeless: true },
      );
      this.log('info', 'patch.library.ready', {
  ...
  async _prepareSlot(renderer, slot, seed, window, descriptor) {
    ...
    const hf = await Heightfield.fromExternal(renderer, seed, {
      ...
      exactGround: window.exactGround,
      exactGroundCapacity: PATCH_RES * PATCH_RES,
      cpuReadback: false,
      progress: (progress, message) => this.log('info', 'patch.hf', { progress, message }),
    });
    ...
    const scatter = await runScatter(renderer, hf, seed, {
      ...
      // Procedural-local +z maps to terrain/EPSG -y.
    ...
    // The application is on Three r184 but renders beneath an ENU/ECEF
    // transform. EqualDepth twins do not remain depth-identical there,
    ...
    root.name = 'atlantis-procedural-clipmap';
  ...
  async _loadClassifier(sourceTiles, descriptor) {
    ...
    for (let start = 0; start < needed.length;) {
      ...
      for (const tile of needed.slice(start, end)) {
        ...
        let chosen = null;
        for (const candidate of classifierSourceCandidates(tile, this.procgenSourceDepth)) {
  ...
  _windowCacheKey(descriptor, sourceTiles, fieldSources) {
    ...
    return [
      ...
      `sourceDepth=${this.procgenSourceDepth}`,
  ...
  async build(renderer, camera, cameraAGL, traversal = {}) {
    ...
    const startedAt = performance.now();
    try {
      const sourceTiles = collectTiles(this.terrainRoot, this.procgenSourceDepth);
      ...
      this.log('info', 'patch.ready', {
        ...
        res: PATCH_RES,
        grounding: window.stats.grounding,
      ...
      this.nextBuildAttempt = now + RETRY_MS;
      this.log('error', 'patch.build', { error: String(err), stack: String(err?.stack ?? '') });
  ...
  async _recenter(renderer, descriptor, sourceTiles) {
    ...
    try {
      ...
      const { window, classifierPages, cacheHit } = resolved;
      // Keep one compiled render graph and rewrite its persistent data buffers
      // in place. A second graph duplicates material/pipeline setup and caused
      // visible multi-second stalls while ordinary simulation was running.
      ...
      // Publish new window metadata only after the persistent buffers have
      // completed their ordered rewrite.
      ...
      this.log('error', 'patch.recenter', { error: String(err), stack: String(err?.stack ?? '') });
    } finally {
      this.reseeding = false;
    }
  }
  ...
  update(renderer, camera, cameraAGL, traversal = {}) {
    ...
    if (this.motionDetailActive && speed >= DETAIL_SPEED_DISABLE_MPS) {
      ...
      this.log('info', 'patch.detail-lod', {
        active: false,
        reason: 'speed',
    ...
    if ((this._diag = (this._diag + 1) % 180) === 0) {
      ...
      this.log('info', 'patch.diag', {
        ...
        plantGroupsSaturated: forestHud['veg.underSaturatedGroups'] ?? -1,
        rockGroupsSaturated: forestHud['veg.extraSaturatedGroups'] ?? -1,
