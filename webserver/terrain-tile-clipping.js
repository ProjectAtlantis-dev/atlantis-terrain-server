...
export function clipTerrainMeshToDescendants(mesh, descendants) {
  ...
  if (priorSignature === nextSignature) {
    const activeIndexCount = geometry.drawRange?.count ?? fullIndexCount;
    Object.assign(mesh.userData, {
  ...
  if (indices.length > indexAttribute.array.length) return false;
...
export function recomputeTerrainResidencyClipping(meshes) {
  ...
  for (let index = 0; index < ordered.length; index += 1) {
    ...
    if (clipTerrainMeshToDescendants(ancestor, descendants)) continue;
