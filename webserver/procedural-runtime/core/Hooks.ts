 ...
 * (verification harnesses read/write `window.__atlantisProcedural`).
 */
...
export interface ProceduralRuntimeHooks {
...
declare global {
  interface Window {
    __atlantisProcedural: ProceduralRuntimeHooks;
  }
}

export function initHooks(): ProceduralRuntimeHooks {
  const hooks: ProceduralRuntimeHooks = {
    ready: false,
    error: null,
    stats: null,
    diag: null,
  ...
  window.__atlantisProcedural = hooks;
  return hooks;
}
