...
export function failLoud(title: string, details: string[]): void {
  if (window.__atlantisProcedural) {
    window.__atlantisProcedural.error = `${title}\n${details.join('\n')}`;
  }
  // eslint-disable-next-line no-console
  console.error('[ATLANTIS PROCEDURAL FATAL]', title, details);
  if (failShown) return;
  failShown = true;
  ...
  el.id = 'atlantis-procedural-fatal';
  el.style.cssText = [
