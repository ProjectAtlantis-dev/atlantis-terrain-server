 ...
 * `window.__atlantisProcedural.stats` for the verification harness.
 */
...
import type { EngineStats, ProceduralRuntimeHooks } from './Hooks';
import type { ProceduralRuntimeParams } from './Params';
...
export class Engine {
  ...
  readonly params: ProceduralRuntimeParams;
  readonly hooks: ProceduralRuntimeHooks;
    ...
    if (device) {
      ...
      device.onuncapturederror = (e: GPUUncapturedErrorEvent): void => {
        if (reported++ < 8) {
          ...
          console.error('[atlantis-procedural] WebGPU uncaptured error:', e.error.message);
        }
      };
    }
