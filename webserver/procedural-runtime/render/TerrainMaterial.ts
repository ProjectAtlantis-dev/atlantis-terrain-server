...
export interface TerrainShadingInputs {
   ...
   */
  baseNormalSlope?: NV4;
  /** Patch-local y-up shading position override (streamed Greenland terrain). */
  worldPosition?: NV3;
  /** Camera in the same patch-local frame as worldPosition. */
...
export function buildTerrainShading(inp: TerrainShadingInputs): TerrainShading {
  ...
  const ridG = (s: number): NV2 =>
    inp.external
      // Atlantis lighting/CSM + satellite + five procedural maps reaches the
  ...
  // WINTER (live uniform): blanket the land in snow — full on gentle-to-moderate
  // ground, thinning only on steep faces (cliffs stay wind-scoured / bare rock).
  // Folded into the snowW expression — material node graphs can't use .assign().
  const snowW = smoothstep(0.16, 0.5, snowField.add(dither))
    .max(seasonU.winter.mul(smoothstep(0.9, 0.4, slope)))
    // SPRING: patchy lingering snowmelt drifts in flat hollows (noise-gated)
    .max(seasonU.spring.mul(smoothstep(0.32, 0.1, slope)).mul(smoothstep(0.6, 0.82, macroA)))
    .toVar();
  ...
  let col: NV3 = soil;
  // GREENLAND: tundra mat — between the sparse grass tufts the ground is living
  // moss + dwarf-shrub, not bare soil (ref land_b: olive moss and rust heath
  // mats carpeting the gaps between rock). Blend a mottled mat (olive moss ↔
  // rust dwarf-shrub) into the ground by veg density on gentle terrain; rock,
  // scree, gravel, pond and snow still override it below.
  // Real Greenland summer/autumn tundra is a MOTTLED carpet: olive moss, warm
  // rust dwarf-shrub, vivid bearberry-scarlet patches, and ochre senescent
  // sedge — the dominant colour of the land between rocks (land_b). Blend all
  // four at different noise scales, gated to gentle vegetated ground.
  const mossGreen = vec3(0.05, 0.11, 0.045);
  const rustHeath = vec3(0.19, 0.085, 0.03);
  const autumnRed = vec3(0.3, 0.06, 0.028);
  const ochreTan = vec3(0.16, 0.13, 0.06);
  const rustN = val(9, 0.31, 0.61);
  const ochreN = val(3.2, 0.53, 0.19);
  let tundraMat = mix(mossGreen, rustHeath, smoothstep(0.32, 0.72, macroA)) as NV3;
  tundraMat = mix(tundraMat, autumnRed, smoothstep(0.6, 0.82, rustN).mul(0.85)) as NV3;
  tundraMat = mix(tundraMat, ochreTan, smoothstep(0.7, 0.92, ochreN).mul(0.45)) as NV3;
  // AUTUMN (live uniform): the whole heath mat shifts toward rust-red (ref
  // image 2). Deeper shift than before so autumn genuinely reads as rust heath.
  tundraMat = mix(tundraMat, autumnRed, seasonU.autumn.mul(0.85)) as NV3;
  // SPRING (live uniform): snowmelt flush — the heath greens up fresh and vivid
  tundraMat = mix(tundraMat, vec3(0.07, 0.17, 0.05), seasonU.spring.mul(0.6)) as NV3;
  // GREENLAND is dwarf-shrub HEATH, so the living mat should CARPET gentle
  // vegetated ground, not just tint it — a 0.35 floor keeps bare tan soil from
  // dominating between the scatter plants (the "grass meadow" look). Steep rock,
  // scree, gravel, pond and snow still override it below.
  const matW = vegDensity
    .mul(0.65)
    .add(0.35)
    .mul(smoothstep(0.62, 0.22, slope))
    .mul(snowField.oneMinus());
  col = mix(col, tundraMat, matW) as NV3;
  col = mix(col, grassCol, grassW);
  col = mix(col, forestFloor, forestW);
  // GREENLAND: exposed glacial BEDROCK sheets — ice-scoured rock showing through
  // the thin tundra on gentle ground (large outcrop patches) and scoured bare
  // along stream channels. Smooth light-grey gneiss with faint banding; this is
  // the rock the boulders + scree sit on.
  const bedrockCol = mix(
    vec3(0.33, 0.33, 0.33),
    vec3(0.46, 0.46, 0.45),
    strata.mul(0.45).add(micro.mul(0.35)).add(0.1),
  ) as NV3;
  const bedrockW = smoothstep(0.55, 0.73, val(30, 0.17, 0.83))
    .max(smoothstep(0.35, 0.7, flowStrength).mul(smoothstep(0.5, 0.2, slope)))
    .mul(smoothstep(0.6, 0.22, slope))
    .mul(snowW.oneMinus());
  col = mix(col, bedrockCol, bedrockW) as NV3;
  ...
  // GREENLAND: crustose GROUND lichen (the catalog lichen type as a terrain
  // decal) — map-lichen yellow-green, grey crustose, and vivid Xanthoria orange
  // patches on dry, exposed, low-veg gravel/soil/scree (not on the wet sward,
  // not under snow). Two noise scales choose patch shape and species.
  const lichenN = val(3.8, 0.29, 0.83);
  const lichenSp = val(0.9, 0.61, 0.13);
  const lichenCol = mix(
    mix(vec3(0.27, 0.31, 0.19), vec3(0.42, 0.41, 0.34), lichenSp),
    vec3(0.56, 0.34, 0.08),
    smoothstep(0.74, 0.9, lichenSp),
  ) as NV3;
  const lichenW = smoothstep(0.54, 0.78, lichenN)
    .mul(smoothstep(0.42, 0.14, moisture))
    .mul(vegDensity.oneMinus().mul(0.6).add(0.25))
    .mul(snowW.oneMinus())
    .mul(0.5);
  col = mix(col, lichenCol, lichenW) as NV3;
