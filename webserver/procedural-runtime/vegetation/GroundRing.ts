...
const G_NEAR = 30;
const G_MID = 70;
/** stochastic LOD transition half-width around each distance boundary */
const G_BAND = 12;
...
// The procedural heightfield is a resampled representation of the rendered DEM.
...
export class GroundRing {
  ...
  init(beechAtlas: DataTexture | null): void {
    ...
    const grassK = Fn(() => {
      ...
      // Industry-style cluster LOD: each world-anchored clump belongs to one
      // geometry LOD. A deterministic cell hash staggers switches across the
      // transition band, so movement changes a few clumps at a time while
      // camera rotation cannot change the selection. This also avoids
      // fragment-mask holes between geometries with different silhouettes.
      const lodHash = cellHash2(wc, salt ^ 0x2c91);
      const nearSwitch = lodHash.x.sub(0.5).mul(G_BAND * 2).add(G_NEAR);
      const midSwitch = lodHash.y.sub(0.5).mul(G_BAND * 2).add(G_MID);
      If(dist.lessThan(nearSwitch), () => {
        appendRing(int(0), wc, h);
      }).Else(() => {
        If(dist.lessThan(midSwitch), () => {
    ...
    // Preserve one silhouette family through the geometric LODs. The former
    // r2 crossed-card tuft was almost three times wider than an r1 clump and
    // produced a camera-centred density line at the 70 m transition.
    const grassGeos = [bladeClump(5, 4), bladeClump(3, 2), bladeClump(3, 1)];
    for (let l = 0; l < 3; l++) {
      ...
      const mat = this.grassMaterial(bindL, false);
      this.patchGI(mat);
    ...
    // terrain-normal shading (mode 'far' in grassMaterial)
    {
      ...
      const matF = this.grassMaterial(bindF, true, true);
      this.patchGI(matF);
  ...
  private grassMaterial(
    ...
    // client. `cameraPosition` is then ECEF while `wpos` is patch-local, which
    ...
    const bladeH = h2.x
      ...
      .mul(0.3)
      .add(0.2)
      // Geometry LOD changes blade/card count and segment count only. Every
      // representation must preserve the same physical plant height; scaling
      // height with distance made the near field look like a flattened circle.
        .mul(1.5)
