...
export class Forests {
  ...
  init(renderer: Renderer): void {
    ...
    for (const pool of lib.pools) {
      ...
      if (isUnderClass(pool.cls)) {
        // Small ground plants use population LOD, not shredded geometry LOD.
        // The previous path drew r0+r1+r2 (160% of the full triangle count)
        // and arbitrary subsets often omitted the upper stem or flower head.
        const full = pool.r0 ?? pool.r1 ?? pool.r2;
        if (full) rings.push({ ring: 0, parts: full });
      } else {
      ...
      for (const { ring, parts } of rings) {
        ...
        for (const part of parts) {
          const mat = part.make();
          const handles = instanceVeg(mat, {
            bufA: layer.bufA,
            bufB: layer.bufB,
            compact: this.compact,
            groupBase: offsets[g] ?? 0,
            fade: isUnderClass(pool.cls) ? null : fadeFor(pool.cls, ring),
            // All LOD meshes use the same stable per-instance threshold.
            // Understory r0/r1/r2 have different triangle sets, so dissolving
            // each surface independently creates view-dependent holes.
            surfaceDither: false,
            wind: windBind,
          ...
          if (isUnderClass(pool.cls)) {
            const maxD = this.lib.clsMaxDist[pool.cls] ?? 150;
            const nearD = Math.min(34, maxD * 0.34);
            const midD = Math.min(82, maxD * 0.7);
            const density = mix(
              mix(float(1), float(0.45), smoothstep(nearD - 8, nearD + 8, handles.dist)),
              float(0.15),
              smoothstep(midD - 12, midD + 12, handles.dist),
            ).mul(float(1).sub(smoothstep(maxD - 18, maxD, handles.dist)));
            mat.maskNode = slotHash(handles.slot, 333).lessThan(density);
          }
          this.patchGI(mat);
  ...
  private async readStats(renderer: Renderer): Promise<void> {
    try {
      ...
      let cast = 0;
      let tris = 0;
      let saturatedUnderGroups = 0;
      let saturatedExtraGroups = 0;
      for (let g = 0; g < GROUPS; g++) {
        ...
        else if (g < EXTRAS_BASE) {
          under += n;
          if (cap > 0 && raw >= cap) saturatedUnderGroups++;
        } else if (g < HERO_BASE) {
          extras += n;
          if (cap > 0 && raw >= cap) saturatedExtraGroups++;
        }
        else hero += n;
      }
      this.hud = {
        ...
        'veg.underSaturatedGroups': saturatedUnderGroups,
        'veg.extraSaturatedGroups': saturatedExtraGroups,
        'veg.cast': cast,
