...
function subsetGeometry(source: BufferGeometry, ratio: number): BufferGeometry {
  ...
  // Procedural leaves and grass cards are emitted as adjacent triangle pairs.
  // Selecting individual triangles tears quads in half; the surviving sliver
  // then appears/disappears as the camera rotates. Reduce complete primitive
  // pairs so every LOD remains a coherent, two-sided plant silhouette.
  const primitiveCount = Math.ceil(triangleCount / 2);
  const keepPrimitives = Math.max(
    1,
    Math.min(primitiveCount, Math.round(primitiveCount * ratio)),
  );
  const selectedPrimitives = Array.from({ length: keepPrimitives }, (_, i) =>
    Math.min(
      primitiveCount - 1,
      Math.floor(((i + 0.5) * primitiveCount) / keepPrimitives),
    ),
  );
  const selected: number[] = [];
  for (const primitive of selectedPrimitives) {
    const first = primitive * 2;
    if (first < triangleCount) selected.push(first);
    if (first + 1 < triangleCount) selected.push(first + 1);
  }
  const keep = selected.length;
...
export async function buildVegLibrary(
  ...
  // weighted by its own habitat via the ArcticFlora `variant` slot.
  ...
  // keeps the procedural runtime affordable. Every understory species gets three real geometry
