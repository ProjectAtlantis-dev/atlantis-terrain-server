...
import type { ProceduralRuntimeParams } from '../core/Params';
...
/** Original streamed terrain vertices covering the active procedural window.
 * Rows retain terrain-mesh-builder's south-up order. `originY` is therefore
 * expressed in terrain-local northing, while Heightfield's local +z points in
 * the opposite direction. */
export interface ExternalGroundGrid {
...
export class Heightfield {
   ...
   * dataset rather than the synthetic-world path. Consumers must not apply
  ...
  // Exact depth-page vertices used by GPU scatter. The 3 m heightfield above
  // remains useful for smooth normals/classifier fields, but resampling it for
  // instance elevation can diverge from the piecewise-planar rendered mesh.
  private externalGroundUpload: StorageBufferAttribute | null = null;
  private externalGround: StorageBufferNode<'float'> | null = null;
  private externalGroundKernel: ComputeNode | null = null;
  private externalGroundParams: ReturnType<typeof uniform> | null = null;
  private externalGroundDims: ReturnType<typeof uniform> | null = null;
  ...
  static async generate(
    renderer: Renderer,
    params: ProceduralRuntimeParams,
    seed: WorldSeed,
    progress: ProgressFn,
  ): Promise<Heightfield> {
   ...
   * the procedural fields/biome/water resources consumed by GroundRing and Forests.
  ...
  static async fromExternal(
    ...
    opts: {
      ...
      cpuReadback?: boolean;
      progress?: ProgressFn;
      exactGround?: ExternalGroundGrid;
      exactGroundCapacity?: number;
    },
  ): Promise<Heightfield> {
    const res = opts.res;
    const N = res * res;
    ...
    if (opts.exactGround) {
      hf.setExternalGround(
        opts.exactGround,
        opts.exactGroundCapacity ?? opts.exactGround.heights.length,
      );
      if (!hf.externalGroundKernel) {
        throw new Error('External ground copy kernel was not initialized');
      }
      await renderer.computeAsync(hf.externalGroundKernel);
    }
  ...
  async reseed(
    ...
    if (exactGround) {
      this.setExternalGround(exactGround);
      if (!this.externalGroundKernel) {
        throw new Error('External ground copy kernel was not initialized');
      }
      await renderer.computeAsync(this.externalGroundKernel);
    }
  ...
  sampleHeight(p: NV2): NF {
    if (this.externalGround && this.externalGroundParams && this.externalGroundDims) {
      const params = this.externalGroundParams as unknown as NV4;
      const dims = this.externalGroundDims as unknown as NV2;
      // Procedural +z is terrain -y. Mapping back to the source page's original
      // row direction preserves terrain-mesh-builder's (a,b,d)/(b,f,d) split.
      const gx = p.x.sub(params.x).mul(params.z);
      const gy = p.y.negate().sub(params.y).mul(params.w);
      const x0f = clamp(floor(gx), 0, dims.x.sub(1));
      const y0f = clamp(floor(gy), 0, dims.y.sub(1));
      const x1f = x0f.add(1).min(dims.x.sub(1));
      const y1f = y0f.add(1).min(dims.y.sub(1));
      const fx = clamp(gx.sub(x0f), 0, 1);
      const fy = clamp(gy.sub(y0f), 0, 1);
      const stride = dims.x.toInt();
      const x0 = x0f.toInt();
      const y0 = y0f.toInt();
      const x1 = x1f.toInt();
      const y1 = y1f.toInt();
      const h00 = this.externalGround.element(y0.mul(stride).add(x0));
      const h10 = this.externalGround.element(y0.mul(stride).add(x1));
      const h01 = this.externalGround.element(y1.mul(stride).add(x0));
      const h11 = this.externalGround.element(y1.mul(stride).add(x1));
      const lower = h00.add(fx.mul(h10.sub(h00))).add(fy.mul(h01.sub(h00)));
      const upper = h10
        .mul(float(1).sub(fy))
        .add(h01.mul(float(1).sub(fx)))
        .add(h11.mul(fx.add(fy).sub(1)));
      return fx.add(fy).lessThanEqual(1).select(lower, upper) as NF;
    }
    return this.sampleHeightFrom(this.height, p);
  }

  private setExternalGround(data: ExternalGroundGrid, capacity?: number): void {
    const required = data.columns * data.rows;
    if (required !== data.heights.length || data.columns < 2 || data.rows < 2) {
      throw new Error('External ground grid has inconsistent dimensions');
    }
    if (!(data.stepX > 0) || !(data.stepY > 0)) {
      throw new Error('External ground grid requires positive spacing');
    }
    if (!this.externalGroundUpload) {
      const allocated = Math.max(required, capacity ?? required);
      const upload = new StorageBufferAttribute(new Float32Array(allocated), 1);
      const source = storage(upload, 'float', allocated);
      const ground = instancedArray(allocated, 'float') as StorageBufferNode<'float'>;
      const copyKernel = Fn(() => {
        const i = instanceIndex;
        If(i.greaterThanEqual(allocated), () => {
          Return();
        });
        ground.element(i).assign(source.element(i));
      })().compute(allocated);
      copyKernel.setName('externalGroundCopy');
      this.externalGroundUpload = upload;
      this.externalGround = ground;
      this.externalGroundKernel = copyKernel;
      this.externalGroundParams = uniform(new Vector4());
      this.externalGroundDims = uniform(new Vector2());
    }
    const target = this.externalGroundUpload.array as Float32Array;
    if (required > target.length) {
      throw new Error(`External ground grid ${required} exceeds capacity ${target.length}`);
    }
    target.set(data.heights, 0);
    this.externalGroundUpload.needsUpdate = true;
    (this.externalGroundParams?.value as Vector4).set(
      data.originX,
      data.originY,
      1 / data.stepX,
      1 / data.stepY,
    );
    (this.externalGroundDims?.value as Vector2).set(data.columns, data.rows);
  }
