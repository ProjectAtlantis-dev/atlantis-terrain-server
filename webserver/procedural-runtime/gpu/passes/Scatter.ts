...
import { GREENLAND_FLORA, type FloraSpecies } from '../../vegetation/ArcticFlora';
...
// ---------------------------------------------------------------------------
// Understory composition — data-driven from the ArcticFlora roster.
...
export async function runScatter(
  ...
  const underK = Fn(() => {
    ...
    // COMPOSITION: data-driven weighted draw over the ArcticFlora roster.
    ...
    append(
      ...
      underA,
      underB,
      // Keep the complete plant silhouette just above the exact rendered
      // triangle. A negative root offset buried short Arctic species.
      vec4(wpos.x, s.h.add(0.03), wpos.y, scale) as unknown as NV4,
