// Adapter: procedural VegLibrary → the AssetLibrary shape that
// procgen/scatter.ts instances. WebGPU client only. Gives the WebGPU scatter the
// full vegetation catalog (30 species, TSL node materials) instead of the old
// stripped GLSL procgen library. Async because the procedural library bakes atlases
...
import { foliageMaterial } from './procedural-runtime/render/VegMaterials.ts';

// VegClass (procedural-runtime/gpu/passes/Scatter.ts) → scatter category prefix. Greenland:
...
export async function buildProceduralAssetLibrary(renderer, seedN) {
  ...
  return { kinds, stats: { kinds: kinds.size, tris: Math.round(tris), buildMs: 0, source: 'atlantis-procedural' } };
}
