...
export function waterMaterial(
  ...
  // ---- WINTER ICE (live seasonU.winter) ---------------------------------------
  // Standing / slow water freezes solid; only genuinely FAST current (rapids)
  // stays open as dark leads. Deep winter freezes lakes, their shallow shores,
  // and slow stream reaches — only fast chutes resist. Threshold runs high so
  // the shallow shore band and slow inlets don't read as open green water.
  const iceAmt = seasonU.winter.mul(smoothstep(0.6, 0.05, spd)).clamp(0, 1);
  ...
  const nWater = vec3(slope.x.negate(), 1, slope.y.negate()).normalize();
  // ice is a flat frozen sheet — kill the ripple tilt where frozen
  const n = mix(nWater, vec3(0, 1, 0), iceAmt).normalize();
  ...
  // ---- compose --------------------------------------------------------------------
  // WINTER ICE surface: patchy snow-white ↔ windswept clear blue ice (macro
  // noise), opaque, matte-rough; blended in by iceAmt so open leads stay water.
  const iceN = (texture(noiseA, positionWorld.xz.div(28 * PERIOD_FBM)) as unknown as NV4).y;
  const iceCol = mix(
    vec3(0.26, 0.4, 0.5),
    vec3(0.82, 0.86, 0.92),
    smoothstep(0.38, 0.62, iceN),
  );
  const waterCol = vec3(0.74, 0.76, 0.74).mul(foam);
  const waterEmis = mix(refr, skyRefl, fres).mul(foam.oneMinus());
  mat.colorNode = mix(waterCol, iceCol, iceAmt);
  // ice keeps only a faint sky sheen; open water keeps full refraction/reflection
  mat.emissiveNode = mix(waterEmis, skyRefl.mul(0.1) as unknown as NV3, iceAmt);
  mat.roughnessNode = mix(mix(float(0.05), float(0.55), foam), float(0.62), iceAmt);
  ...
  const waterOpac = smoothstep(0.004, 0.05, vDepth).mul(rampK).mul(0.985);
  // ice is an opaque sheet (hides the bed) except a thin feather at the very edge
  const iceOpac = smoothstep(0.002, 0.02, vDepth).mul(rampK);
  mat.opacityNode = mix(waterOpac, iceOpac, iceAmt);
